classdef CrewPersonImplEVA < handle
...
%         DryWasteProducerDefinition = ResourceUseDefinitionImpl
    ...
    properties (SetAccess = private)
        ...
        O2HighTillDead = 24*3600             % seconds exposed at high O2 levels (with fire risk) until dead
        O2HighRecoveryRate = 0.01/3600
        O2LowPartialPressure = 15.168             % kPa, absolute minimum ppO2 that a person should be exposed to - REF- HIDH  pg 324, Table 6.2-5
        O2LowTillDead = 2*3600               % seconds spent in a low O2 level until death occurs (corresponds to amount of internal buffer that the crew has)
        O2LowRecoveryRate = 0.01/3600
        ...
        TotalPressureLowTillDead = 1*3600            % Seconds spent in a low total pressure condition until death occurs
    ...
    methods
        %% Constructor
        function obj = CrewPersonImplEVA(name,age,weight,sex,schedule)
...
%                 resultinLiters = (a + b*heartRate^3) * 60;  %in L/hr    Correct version
                resultinLiters = (a + b*heartRate^3) / 60;  %in L/sec    
...
%                 O2needed = resultinMoles;
        ...
        function potableWaterNeeded = calculateCleanWaterNeeded(obj)
            potableWaterNeeded = 0.1667/3600*(obj.CurrentActivity.Intensity >= 0);
        end
...
%                         kJneeded = 106*obj.Weight + 6067*activityCoefficient;   % Incorrect version of equation used in BioSim
                ...
                caloriesNeeded = kJneeded/(24*3600)/4.184;   % Convert from kJ/person/day to calories/person/hour (a more accurate conversion factor would be 4.184) (actually this should be in kcal)
            end          
