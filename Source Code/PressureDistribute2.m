classdef PressureDistribute2 < handle
    %PressureDistribute2 Summary of this class goes here
    ...
    methods
        %% Constructor
        function obj = PressureDistribute2(modules,adjacencyMatrix)
...
%             moleDiffVector = [molarDiff';zeros(sum(sum(obj.AdjacencyMatrix))/2,1)];
            ...
            pressureDiffVector = [pressureDiff';zeros(sum(sum(obj.AdjacencyMatrix))/2,1)];
            
            % Solve system of equations
            pressureExchange = obj.LinearSystem\pressureDiffVector;
            
            % Take only positive values within solution (corresponding to
            % flowing from one module to another) and calculate moles to
            % take (all together)
                        
            airGivers = obj.SourceNodes(pressureExchange>0);
            pressureToMove = pressureExchange(pressureExchange>0);
            airTakers = obj.SinkNodes(pressureExchange>0);
            
            % Take air from each module
            takenAir = zeros(length(airGivers),5);
            
            for j = 1:length(airGivers)
                takenAir(j,:) = obj.takeAir(obj.Modules(airGivers(j)),pressureToMove(j));
            end
        ...
        function takenAir = takeAir(obj,module,deltaP)
            takenAir = zeros(1,5);
            takenAir(1) = module.O2Store.take(deltaP*module.O2Percentage*module.volume/(obj.idealGasConstant*(module.temperature+obj.CelsiusToKelvin)));
            takenAir(2) = module.CO2Store.take(deltaP*module.CO2Percentage*module.volume/(obj.idealGasConstant*(module.temperature+obj.CelsiusToKelvin)));
            takenAir(3) = module.NitrogenStore.take(deltaP*module.N2Percentage*module.volume/(obj.idealGasConstant*(module.temperature+obj.CelsiusToKelvin)));
            takenAir(4) = module.VaporStore.take(deltaP*module.VaporPercentage*module.volume/(obj.idealGasConstant*(module.temperature+obj.CelsiusToKelvin)));
            takenAir(5) = module.OtherStore.take(deltaP*module.OtherPercentage*module.volume/(obj.idealGasConstant*(module.temperature+obj.CelsiusToKelvin)));
        end
